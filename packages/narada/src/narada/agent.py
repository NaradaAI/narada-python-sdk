...
class Agent(Generic[_StructuredOutput]):
    ...
    async def reset_agent_state(self) -> None:
        env = self._browser_environment()
        if not hasattr(env, "reset_agent_state"):
            raise ValueError(
                f"{type(self.environment).__name__} does not support agent state reset"
            )
        await env.reset_agent_state()  # type: ignore[attr-defined]
